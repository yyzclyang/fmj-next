...
- 代码的清晰简洁远大于性能，现代设备对于这个项目的需求性能远远足够。
- TypeScript 重写时优先保持模块边界与原工程可映射，例如 `combat`、`scene`、`script` 等目录语义尽量稳定，便于逐步迁移与交叉校验。
- 类型名使用 `PascalCase`，函数与变量使用 `camelCase`；屏幕、动作、脚本进程等核心概念可延续旧工程命名习惯，如 `ScreenMainGame`、`ActionDefend`、`ScriptVM`，前提是不会明显违背 TS 侧现有风格。
- 修改时只格式化直接改动的部分，不整理无关旧代码。
- C 代码、反汇编资料和 Kotlin 旧实现默认都只作为参考，不做顺手重构。
- 添加合适的注释帮助解释逻辑。
