...
if (playerAgility > enemyAgility) {
    ...
    agilityDiff = 10; // 玩家没有身法优势时，按固定差值处理
}

// 命中判定：random % 200 < agilityDiff 时 MISS
...
| **判定方向** | `hitRoll < agilityDiff` 时未命中 | 基于概率直接判定 |
| **命中范围** | `random % 200` 的整数判定 | 10%-90%（固定范围） |
| **计算方式** | 整数运算，`% 200` | 浮点运算，概率计算 |
| **默认差值** | 玩家无身法优势时固定为10 | 10%保底 |
...
```c
// 代码先计算 100 - 原始智商值
UINT8 normalAttackThreshold = 100 - enemy.rawIQ;
...
if (random < normalAttackThreshold) {
    ...
    // 尝试释放魔法；若无魔法链、MP不足或状态异常，再回退为普通攻击
    selectMagicToUse();
}
```
...
| 方面 | 原版算法 | 简化算法 |
|------|---------|---------|
| **字段用法** | 先算 `100 - 原始值` 作为普通攻击阈值 | `原始值 / 100` |
| **施法概率** | 满足施法前提时约为 `原始值 / 100` | 0%-100% |
| **决策因素** | 仅智商 | 智商+血量+魔法类型 |
| **随机性** | 纯随机，失败时回退普通攻击 | 策略性随机 |
...
const val DEFAULT_AGILITY_DIFF = 10     // 玩家无身法优势时的固定差值
const val SPIRIT_DAMAGE_SHIFT = 6       // 灵力影响位移值 (1/64)
const val RANDOM_DAMAGE_SHIFT = 4       // 随机浮动位移值 (1/16)
const val SPECIAL_DEFENSE_MAGIC_SHIFT = 2 // 敌人魔法 HP 伤害的特减位移值 (1/4 = 25%)
```
